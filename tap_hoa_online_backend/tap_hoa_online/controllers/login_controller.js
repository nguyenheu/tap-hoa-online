...
module.exports.controller = (app) => {
    ...
    const msg_already_register = "Email này đã được đăng ký trước đó";

    const msg_added_favorite = "Thêm vào danh sách yêu thích thành công";
    const msg_removed_favorite = "Xóa khỏi danh sách yêu thích thành công";

    const msg_invalid_item = "Sản phẩm không hợp lệ";
    const msg_add_to_item = "Thêm sản phẩm vào giỏ hàng thành công";
    const msg_remove_to_cart = "Xóa sản phẩm khỏi giỏ hàng thành công";

    const msg_add_address = "Thêm địa chỉ thành công";
    const msg_update_address = "Cập nhật địa chỉ thành công";
    const msg_remove_address = "Xóa địa chỉ thành công";

    const msg_add_payment_method = "Thêm phương thức thanh toán thành công";
    const msg_remove_payment_method = "Xóa phương thức thanh toán thành công";
    ...
    app.post('/api/app/home', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (uObj) => {
            ...
            db.query("SELECT `od`.`price` as `offer_price`, `od`.`start_date`, `od`.`end_date`, `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`,(CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" + image_base_url + "' ,'', `imd`.`image`) ELSE '' END) AS `image`, `cd`.`cat_name`, (CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav` FROM `offer_detail` AS `od` " +
                ...
                "SELECT `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, (CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" + image_base_url + "' ,'', `imd`.`image` ) ELSE '' END) AS `image`, `cd`.`cat_name`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav` FROM  `product_detail` AS `pd` " +
                "LEFT JOIN  `favorite_detail` AS `fd` ON  `pd`.`prod_id` = `fd`.`prod_id` AND `fd`.`user_id` = ? AND `fd`.`status`=  1 " +
                "INNER JOIN `image_detail` AS `imd` ON `pd`.`prod_id` = `imd`.`prod_id` AND `imd`.`status` = 1 " +
                "INNER JOIN `category_detail` AS `cd` ON `cd`.`cat_id` = `pd`.`cat_id` AND `cd`.`status` = 1 " +
    ...
    app.post('/api/app/favorite_list', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            ...
            db.query("SELECT `fd`.`fav_id`, `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, `pd`.`created_date`, `pd`.`modify_date`, `cd`.`cat_name`, IFNULL(`od`.`price`, `pd`.`price` ) as `offer_price`, IFNULL(`od`.`start_date`,'') as `start_date`, IFNULL(`od`.`end_date`,'') as `end_date`, (CASE WHEN `od`.`offer_id` IS NOT NULL THEN 1 ELSE 0 END) AS `is_offer_active`, 1 AS `is_fav`, (CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" + image_base_url + "' ,'', `imd`.`image` ) ELSE '' END) AS `image` FROM `favorite_detail` AS  `fd` " +
                                ...
                                if (reqObj.promo_code_id != "") {
                                    if (pResult[1].length > 0) {
                                        ...
                                        if (final_total > pResult[1][0].min_order_amount) {
                                            ...
                                            res.json({
                                                'status': "0"
                                                , 'message': "Đơn tối thiểu để áp dụng mã giảm giá: " + pResult[1][0].min_order_amount + "K"
                                            })
                                            return
                                        }
                                        ...
                                        res.json({
                                            'status': "0"
                                            , 'message': "Oops, mã giảm giá không được áp dụng"
                                        })
                                        return
                                    }
                                }
                                        ...
                                        if (result) {
                                            ...
                                            if (reqObj.payment_type == "1") {
                                                ...
                                                db.query("INSERT INTO `notification_detail`( `ref_id`, `user_id`, `title`, `message`, `notification_type`) VALUES (?,?,?, ?,?)", [result.insertId, userObj.user_id,
                                                    "Order Placed", "your order #" + result.insertId + " placed.", "2"], (err, iResult) => {
                                                        ...
                                                        if (iResult) {
                                                            helper.Dlog("Thông báo được thêm thành công")
                                                        } else {
                                                            helper.Dlog("Lỗi")
                                                        }
                                                    })
                                            ...
                                            db.query("UPDATE `cart_detail` SET `status`= 2 ,`modify_date`= NOW() WHERE `user_id` = ? AND `status`= 1 ", [userObj.user_id], (err, cResult) => {
                                                ...
                                                if (cResult.affectedRows > 0) {
                                                    helper.Dlog("Xoá giỏ hàng thành công")
                                                } else {
                                                    helper.Dlog("Lỗi xoá giỏ hàng")
                                                }
                                            })
                    ...
                    if (result) {
                        ...
                        db.query("UPDATE `order_detail` SET `payment_status`=?,`modify_date`= NOW() WHERE `order_id` = ? AND `user_id` = ? AND `status` = 1", [reqObj.payment_status == "1" ? "2" : "3", reqObj.order_id, userObj.user_id], (err, uResult) => {
                            ...
                            if (uResult.affectedRows > 0) {
                                ...
                                helper.Dlog("Cập nhật trạng thái thanh toán đơn hàng thành công")
                            } else {
                                helper.Dlog("Lỗi cập nhật trạng thái thanh toán đơn hàng")
                            }
                        })
                        res.json({
                            'status': "1"
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            helper.CheckParameterValid(res, reqObj, ["order_id"], () => {
                ...
                db.query("SELECT `od`.`order_id`, `od`.`cart_id`, `od`.`total_price`, `od`.`user_pay_price`, `od`.`discount_price`, `od`.`deliver_price`, `od`.`deliver_type`, `od`.`payment_type`, `od`.`payment_status`, `od`.`order_status`, `od`.`status`, `od`.`created_date` FROM `order_detail` AS `od` " +
                    ...
                    "SELECT `uod`.`order_id`, `ucd`.`cart_id`, `ucd`.`user_id`, `ucd`.`prod_id`, `ucd`.`qty`, `pd`.`cat_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, `pd`.`created_date`, `pd`.`modify_date`, `cd`.`cat_name`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav`, IFNULL(`od`.`price`, `pd`.`price` ) as `offer_price`, IFNULL(`od`.`start_date`,'') as `start_date`, IFNULL(`od`.`end_date`,'') as `end_date`, (CASE WHEN `od`.`offer_id` IS NOT NULL THEN 1 ELSE 0 END) AS `is_offer_active`, (CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" + image_base_url + "' ,'', `imd`.`image` ) ELSE '' END) AS `image`, (CASE WHEN `od`.`price` IS NULL THEN `pd`.`price` ELSE `od`.`price` END) as `item_price`, ( (CASE WHEN `od`.`price` IS NULL THEN `pd`.`price` ELSE `od`.`price` END) * `ucd`.`qty`)  AS `total_price` FROM `order_detail` AS `uod` " +
    ...
    function getProductDetail(res, prod_id, user_id) {
        db.query("SELECT `pd`.`prod_id`, `pd`.`cat_id`, `pd`.`name`, `pd`.`detail`, `pd`.`unit_name`, `pd`.`unit_value`, `pd`.`nutrition_weight`, `pd`.`price`, `pd`.`created_date`, `pd`.`modify_date`, `cd`.`cat_name`, ( CASE WHEN `fd`.`fav_id` IS NOT NULL THEN 1 ELSE 0 END ) AS `is_fav`, IFNULL(`od`.`price`, `pd`.`price` ) as `offer_price`, (CASE WHEN `imd`.`image` != '' THEN  CONCAT( '" + image_base_url + "' ,'', `imd`.`image` ) ELSE '' END) AS `image`, IFNULL(`od`.`start_date`,'') as `start_date`, IFNULL(`od`.`end_date`,'') as `end_date`, (CASE WHEN `od`.`offer_id` IS NOT NULL THEN 1 ELSE 0 END) AS `is_offer_active` FROM `product_detail` AS  `pd` " +
            "INNER JOIN `category_detail` AS `cd` ON `pd`.`cat_id` = `cd`.`cat_id` " +
            "INNER JOIN `image_detail` AS `imd` ON `pd`.`prod_id` = `imd`.`prod_id` AND `imd`.`status` = 1 " +
            "LEFT JOIN  `favorite_detail` AS `fd` ON  `pd`.`prod_id` = `fd`.`prod_id` AND `fd`.`user_id` = ? AND `fd`.`status`=  1 " +
            "LEFT JOIN `offer_detail` AS `od` ON `pd`.`prod_id` = `od`.`prod_id` AND `od`.`status` = 1 AND `od`.`start_date` <= NOW() AND `od`.`end_date` >= NOW() " +
