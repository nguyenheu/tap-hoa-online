...
//        self.countryObj = Country(phoneCode: txtMobileCode.replacingOccurrences(of: "+", with: ""), isoCode: "IN")
    ...
    func serviceCallUpdate(){
        ...
        if(txtMobile.isEmpty) {
            self.errorMessage = "Vui lòng nhập số điện thoại"
            self.showError = true
            return
        }
        
        if(txtUsername.isEmpty) {
            self.errorMessage = "Vui lòng nhập user name"
            self.showError = true
            return
        }
    ...
    func serviceCallChangePassword(){
        ...
        if(txtCurrentPassword.isEmpty) {
            self.errorMessage = "Nhập mật khẩu hiện tại"
            self.showError = true
            return
        }
        
        if(txtNewPassword.count < 6) {
            self.errorMessage = "Mật khẩu tối thiểu 6 ký tự"
            self.showError = true
            return
        }
        
        if(txtNewPassword != txtConfirmPassword) {
            self.errorMessage = "Mật khẩu không trùng nhau"
            self.showError = true
            return
        }
